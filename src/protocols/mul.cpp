...
namespace shark {
    namespace protocols {
        namespace mul {
            ...
            void gen(const shark::span<u32> &r_X, const shark::span<u32> &r_Y, shark::span<u32> &r_Z)
            {
                u64 n = r_X.size();
                always_assert(r_Y.size() == n);
                always_assert(r_Z.size() == n);
            ...
            void eval(const shark::span<u32> &X, const shark::span<u32> &Y, shark::span<u32> &Z)
            {
                ...
                Z = authenticated_reconstruct_32(Z_share, Z_tag);
            }
