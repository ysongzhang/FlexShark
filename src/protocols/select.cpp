...
namespace shark
{
    namespace protocols
    {
        namespace select
        {
            ...
            void gen(const shark::span<u8> &r_s, const shark::span<u32> &r_X, shark::span<u32> &r_Y)
            {
                always_assert(r_X.size() == r_s.size());
                always_assert(r_Y.size() == r_s.size());
                randomize(r_Y);

                shark::span<u32> u(r_X.size());
                shark::span<u32> v(r_X.size());
                shark::span<u32> w(r_X.size());
                shark::span<u32> z(r_X.size());
            ...
            void gen(const shark::span<u8> &r_s, const shark::span<u16> &r_X, shark::span<u16> &r_Y)
            {
                always_assert(r_X.size() == r_s.size());
                always_assert(r_Y.size() == r_s.size());
                randomize(r_Y);

                shark::span<u16> u(r_X.size());
                shark::span<u16> v(r_X.size());
                shark::span<u16> w(r_X.size());
                shark::span<u16> z(r_X.size());
