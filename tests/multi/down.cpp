...
u64 double2fix_64(double x) {
    return static_cast<u64>(static_cast<std::make_signed_t<u64>>(x * (1 << FLOAT_PRECISION)));  // Note: must truncate the decimal part using static_cast<int_t>
}
...
double fix2double(u64 x) {
    return static_cast<double>(static_cast<std::make_signed_t<u64>>(x)) / (1 << FLOAT_PRECISION);
}
